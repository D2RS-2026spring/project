...
#【数据读取器】负责检查文件是否存在，并使用 pandas 将 CSV 转换为 NumPy 矩阵。
...
#【随机种子固定】确保每次运行代码得到的结果是一样的，这对于科学实验的“可重复性”至关重要。
...
def fitness_fcn(x: np.ndarray,
    ...
    【适应度函数 / 目标函数】
    算法的“核心评估器”：
    1. 调用 forward 进行物理模拟得到预测值。
    2. 调用 loss 计算预测值与真实值的差距。
    优化算法的目标就是让这个返回的 float 值越小越好。
    """
...
#【PSO 专用包装器】粒子群算法会同时测试多组参数(xs)，所以这里循环计算每一组参数的损失。
